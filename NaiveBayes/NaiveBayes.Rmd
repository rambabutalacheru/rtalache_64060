...
```{r}
UB<-read.csv("C:/Users/ramne/Desktop/ML Assignment/Naive Bayes/UniversalBank.csv")
```
...
A)Create a pivot table for the training data with Online as a column variable, CC as a row variable
  and Loan as a secondary row variable such that the values inside the table convey the count.
  
```{r}
melt_tbank<- melt(tdata,id=c("CreditCard","Personal.Loan"),variable="Online")
cast_tbank<-dcast(melt_tbank,CreditCard+Personal.Loan~Online)
cast_tbank[,c(1:2,14)]
```
